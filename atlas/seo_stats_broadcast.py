...
# SSL verification: default to True (secure). Set BEACON_SSL_VERIFY=false to disable.
SSL_VERIFY = os.environ.get("BEACON_SSL_VERIFY", "true").lower() not in ("0", "false", "no")
...
def fetch_all_agents() -> List[Dict]:
    ...
    # Relay agents via discover
    try:
        resp = requests.get(f"{RELAY_HOST}/beacon/relay/discover?include_dead=true", timeout=10, verify=_SSL_VERIFY)
        if resp.ok:
            relay = resp.json()
            if isinstance(relay, list):
                for a in relay:
    ...
    # Native agents — parse from sitemap
    try:
        resp = requests.get(f"{BASE_URL}/sitemap.xml", timeout=10, verify=_SSL_VERIFY)
        if resp.ok:
            import re
            urls = re.findall(r"<loc>.*?/beacon/agent/([^<]+)</loc>", resp.text)
            relay_ids = {a["agent_id"] for a in agents}
...
def check_agent_seo(agent_id: str) -> Dict:
    ...
    # Check HTML profile
    try:
        resp = requests.get(f"{BASE_URL}/agent/{agent_id}", timeout=8, verify=_SSL_VERIFY)
        if resp.ok:
            html = resp.text
            stats["html_profile"] = True
            stats["schema_org"] = "schema.org" in html and "SoftwareApplication" in html
    ...
    # Check JSON profile
    try:
        resp = requests.get(f"{BASE_URL}/agent/{agent_id}.json", timeout=5, verify=_SSL_VERIFY)
        stats["json_profile"] = resp.ok
    except Exception:
        pass
    ...
    # Check XML profile
    try:
        resp = requests.get(f"{BASE_URL}/agent/{agent_id}.xml", timeout=5, verify=_SSL_VERIFY)
        stats["xml_profile"] = resp.ok
    except Exception:
        pass
...
def check_directory_and_sitemap(agent_ids: List[str]) -> Dict[str, Dict[str, bool]]:
    ...
    try:
        resp = requests.get(f"{BASE_URL}/directory", timeout=10, verify=_SSL_VERIFY)
        if resp.ok:
            for aid in agent_ids:
                if aid in resp.text:
                    presence[aid]["in_directory"] = True
    ...
    try:
        resp = requests.get(f"{BASE_URL}/llms.txt", timeout=10, verify=_SSL_VERIFY)
        if resp.ok:
            for aid in agent_ids:
                if aid in resp.text:
                    presence[aid]["in_llms_txt"] = True
